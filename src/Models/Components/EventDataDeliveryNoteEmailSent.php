<?php

/**
 * Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.
 */

declare(strict_types=1);

namespace Factuarea\Sdk\Models\Components;


/** EventDataDeliveryNoteEmailSent - Payload (`data`) emitted with the `delivery_note.email_sent` event: the full resource snapshot captured at emission time under `object`, plus event-specific keys. */
class EventDataDeliveryNoteEmailSent
{
    /**
     * A delivery note tracking goods delivered to a customer. The delivery address fields, `transport_details` and `reference_number` are both accepted on write and returned here; `delivery_city`, `delivery_province` and `delivery_postal_code` are also filterable on `GET /v1/delivery_notes`. Empty strings are normalized to `null`.
     *
     * @var \Factuarea\Sdk\Models\Components\DeliveryNote $object
     */
    #[\Speakeasy\Serializer\Annotation\SerializedName('object')]
    #[\Speakeasy\Serializer\Annotation\Type('\Factuarea\Sdk\Models\Components\DeliveryNote')]
    public DeliveryNote $object;

    /**
     * $cc
     *
     * @var array<string> $cc
     */
    #[\Speakeasy\Serializer\Annotation\SerializedName('cc')]
    #[\Speakeasy\Serializer\Annotation\Type('array<string>')]
    public array $cc;

    /**
     * $bcc
     *
     * @var array<string> $bcc
     */
    #[\Speakeasy\Serializer\Annotation\SerializedName('bcc')]
    #[\Speakeasy\Serializer\Annotation\Type('array<string>')]
    public array $bcc;

    /**
     *
     * @var ?string $recipientEmail
     */
    #[\Speakeasy\Serializer\Annotation\SerializedName('recipient_email')]
    public ?string $recipientEmail;

    /**
     *
     * @var ?string $deliveryNoteNumber
     */
    #[\Speakeasy\Serializer\Annotation\SerializedName('delivery_note_number')]
    public ?string $deliveryNoteNumber;

    /**
     *
     * @var string $type
     */
    #[\Speakeasy\Serializer\Annotation\SerializedName('type')]
    public string $type;

    /**
     * @param  string  $type
     * @param  \Factuarea\Sdk\Models\Components\DeliveryNote  $object
     * @param  array<string>  $cc
     * @param  array<string>  $bcc
     * @param  ?string  $recipientEmail
     * @param  ?string  $deliveryNoteNumber
     * @phpstan-pure
     */
    public function __construct(DeliveryNote $object, array $cc, array $bcc, ?string $recipientEmail = null, ?string $deliveryNoteNumber = null, string $type = 'delivery_note.email_sent')
    {
        $this->object = $object;
        $this->cc = $cc;
        $this->bcc = $bcc;
        $this->recipientEmail = $recipientEmail;
        $this->deliveryNoteNumber = $deliveryNoteNumber;
        $this->type = $type;
    }
}